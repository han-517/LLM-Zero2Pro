...
> 课程定位：最后一周不再堆叠新名词，而是把 Dense、现代 Attention、MoE、训练和推理系统放进一个可证伪问题。项目必须交付代码、测试、原始记录、图表、失败案例与口述解释。完整 rubric 与目录建议见扩展阅读 [毕业项目说明](../03_capstone.md)；本讲义负责把它变成执行和验收契约。
...
需要完成核心 Notebook 和至少一个端到端模型实验。按选题复习 [现代解码器 Notebook](../../notebooks/core/06_modern_decoder.ipynb)、[注意力前沿 Notebook](../../notebooks/core/08_attention_frontiers.ipynb)、[MoE Notebook](../../notebooks/core/09_moe.ipynb) 与 [推理 Notebook](../../notebooks/core/11_inference_serving.ipynb)。先阅读 [毕业项目说明](../03_capstone.md)，它是本周扩展阅读和最终目录/rubric 依据。不要为了“新颖”临时引入无法测试的大模型或云服务。
...
先从 [毕业项目说明](../03_capstone.md) 选择一个最小问题并写预注册表。架构对照使用 [架构演化互动图](../interactive/architecture-evolution.html)，Attention/MoE 分别对照 [注意力 Notebook](../../notebooks/core/08_attention_frontiers.ipynb) 与 [MoE Notebook](../../notebooks/core/09_moe.ipynb)，服务指标对照 [推理 Notebook](../../notebooks/core/11_inference_serving.ipynb) 和 [服务互动图](../interactive/serving-lab.html)。按选题完成 [starter 16](../../exercises/starter/16_attention_frontiers.py)、[starter 18](../../exercises/starter/18_moe_systems.py) 或 [starter 20](../../exercises/starter/20_inference_systems.py)。
...
- 最终目录和 rubric 逐项对照 [毕业项目说明](../03_capstone.md)，他人可从零复现。
