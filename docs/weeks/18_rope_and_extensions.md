...
在 `docs/interactive/architecture-lab.html` 调节相对位移和旋转比例；在 `notebooks/core/06_modern_decoder.ipynb` 画多频率相位并核对同移不变性；完成 `exercises/starter/07_rope.py`。互动单位圆只画一个频带，不能替代完整 head 的频谱实验。
