...
在 `docs/interactive/architecture-lab.html` 切换 `MLA baseline`，观察边界提示；在 `notebooks/core/08_attention_frontiers.ipynb` 比较 dense/GQA/latent bytes 与重建 MAC；完成 starter `17` 的 latent-cache 部分。互动图只画一条 latent，不表示完整吸收路径。
