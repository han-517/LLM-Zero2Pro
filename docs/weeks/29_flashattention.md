...
使用 `notebooks/core/08_attention_frontiers.ipynb` 比较 dense 与 tiled 输出、梯度、理论中间字节；完成 starter `16` 的 online softmax。`docs/interactive/architecture-evolution.html` 用于区分 IO、cache、稀疏和状态路线，不把 FlashAttention 放进 KV 格式下拉框。
