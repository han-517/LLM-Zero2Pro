...
使用 `notebooks/core/06_modern_decoder.ipynb` 记录 LayerNorm/RMSNorm 的输出均值、RMS 和梯度；在 `docs/interactive/training-and-alignment.html` 观察训练内存与数值策略；完成 starter `09` 中 RMSNorm 部分。互动图负责形成直觉，starter 与 PyTorch 公式 oracle 才负责验收。
