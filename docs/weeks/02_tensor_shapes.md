...
运行 `notebooks/core/01_shapes_and_autograd.ipynb` 的形状单元，并打开 `docs/interactive/core-concepts.html` 与 `docs/interactive/foundations-lab.html`。本周没有独立填空模板；第 3 周的 `exercises/starter/11_autograd.py` 会立即复用本周形状与分支语义。阶段导航见 `docs/stages/01_foundations.md`。
