...
运行 `notebooks/core/02_neural_language_models.ipynb` 的 bigram 与固定窗口 MLP；打开 `docs/interactive/foundations-lab.html`，在 Bigram/MLP/RNN 间切换观察可见上下文。补完 `exercises/starter/12_neural_lm.py` 的 `bigram_counts`，使用 `uv run llm-course exercises check 12` 核查。核心 API 在 `src/llm_from_scratch/neural_lm.py`。
