...
在 `docs/interactive/architecture-lab.html` 调窗口与层数；使用 `notebooks/core/08_attention_frontiers.ipynb` 画 mask、邻接和多层可达性；完成 starter `16` 的 sliding-window 部分。互动条形图是上界直觉，正式实验用图算法核查。
