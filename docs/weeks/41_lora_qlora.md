...
运行 `notebooks/core/10_posttraining.ipynb` 的 LoRA 单元，打开 `docs/interactive/training-and-alignment.html` 理解训练内存构成。使用 `exercises/starter/19_posttraining.py` 的 sequence log-prob/组优势作为后续衔接；LoRA 核心可对照 `src/llm_from_scratch/post_training.py`。实验比较 r=1/2/4/8 对一个已知矩阵差值的拟合误差、可训练参数与 merge 误差，先预测秩上界再运行。
