...
需要理解自回归 prefill/decode、每层 K/V shape、因果 attention 和 KV cache。先复习 [推理 Notebook](../../notebooks/core/11_inference_serving.ipynb) 中缓存预算，再打开 [服务互动图](../interactive/serving-lab.html) 观察上下文长度与并发。无需掌握 CUDA page table；本周的“page”是服务引擎管理的固定 token block，不是操作系统硬件页错误机制。
...
在 [推理 Notebook](../../notebooks/core/11_inference_serving.ipynb) 先手算单 token KV 字节；到 [服务互动图](../interactive/serving-lab.html) 改变并发、上下文和 page size；在 [starter 20](../../exercises/starter/20_inference_systems.py) 完成页分配与连续调度。实验一随机生成序列长度，比较固定最大长度、连续分配与分页的浪费；实验二扫描 B∈{4,8,16,32}，画内部浪费和页表项；实验三模拟长 prompt 与短 decode 混合到达，比较请求级 batching 与每轮准入的完成时间。运行前必须写下预测及公平策略。
