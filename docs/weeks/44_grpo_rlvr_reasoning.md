...
运行 `notebooks/core/10_posttraining.ipynb` 的 group advantage 与 toy verifier 单元，打开 `docs/interactive/training-and-alignment.html` 切到 GRPO/RLVR。补完 `exercises/starter/19_posttraining.py` 的 `group_advantages` 并运行 `uv run llm-course exercises check 19`。实验扫描 G 与基础成功率 p，测有效组比例；再故意攻击算术答案解析器，提交至少三条 reward hacking 样例及修复测试。
