...
在 `docs/interactive/training-and-alignment.html` 调节过滤漏斗；使用 `notebooks/core/07_pretraining_systems.ipynb` 生成来源、语言、长度和许可状态统计；完成 starter `14` 的 data card/manifest 部分。Notebook 只处理课程小数据，不能声称覆盖互联网规模治理。
