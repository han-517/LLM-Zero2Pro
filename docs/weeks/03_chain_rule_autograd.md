...
运行 `notebooks/core/01_shapes_and_autograd.ipynb` 的 finite difference 与分支图单元；打开 `docs/interactive/foundations-lab.html` 调整 x、y 观察两支梯度。实现 `exercises/starter/11_autograd.py` 的 `branch_gradients`，用 `uv run llm-course exercises check 11` 核查。可对照 `src/llm_from_scratch/autograd.py`，但应先独立完成 starter。
