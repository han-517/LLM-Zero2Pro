...
先运行 `notebooks/00_START_HERE.ipynb`，再进入 `notebooks/core/01_shapes_and_autograd.ipynb`。互动入口是 `docs/interactive/index.html`；本周无填空 starter，验证入口是 `uv run llm-course doctor` 与 `uv run pytest -q`。阶段概览 `docs/stages/01_foundations.md` 用作导航，本讲义才是第 1 周的唯一 lecture。
