...
在 `notebooks/core/07_pretraining_systems.ipynb` 比较 dtype 误差、理论/实测字节与 checkpoint 重计算；在 `docs/interactive/training-and-alignment.html` 查看每设备内存分项。本周没有独立 starter 时，把资源计算器、AMP 正反例和 profiler 表作为交付物。
