...
先掌握 embedding lookup、矩阵乘法、sin/cos 和 self-attention 的粗略流程；注意力细节在第 12 周推导。主实验为 `notebooks/core/04_attention_mechanics.ipynb`，互动图是 `docs/interactive/core-concepts.html` 与 `docs/interactive/architecture-evolution.html`。本周没有必填 starter；不要把第 7 号 RoPE starter 当作绝对位置作业，它属于后续现代 decoder 深化。
...
在 `notebooks/core/04_attention_mechanics.ipynb` 完成“打乱 token”与绝对位置相加实验；打开 `docs/interactive/core-concepts.html`，固定 Q/K/V 后只改变位置相关输入，观察 score/heatmap；再用 `docs/interactive/architecture-evolution.html` 对照 absolute、relative、RoPE、ALiBi 的注入点。本周路线没有必填 starter，产出是一个能证明无位置时置换等变的最小反例及其数值结果。
