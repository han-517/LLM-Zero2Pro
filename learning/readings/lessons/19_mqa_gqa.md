...
在 `learning/readings/interactive/architecture-lab.html` 切换 MHA/MQA/GQA 并改变序列长度；在 `learning/labs/06_modern_decoder.ipynb` 输出缓存 shape 与字节数；完成 `learning/labs/starter/08_grouped_query_attention.py`。互动估算默认一层一 batch，只用于直觉，正式交付必须调用通用预算函数。
