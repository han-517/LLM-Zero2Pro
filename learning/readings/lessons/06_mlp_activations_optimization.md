...
运行 `learning/labs/02_neural_language_models.ipynb` 的 MLP、激活与梯度流部分；打开 `learning/readings/interactive/foundations-lab.html` 比较上下文模型。实现 `learning/labs/starter/12_neural_lm.py` 前先完成本周无 `nn.Sequential` 的 MLP；参考实现接口位于 `src/llm_from_scratch/neural_lm.py`，不要复制答案。
