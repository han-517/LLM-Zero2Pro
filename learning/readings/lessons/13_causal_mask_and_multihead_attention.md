...
需完成第 12 周，掌握 `[B,H,T,Dh]` attention 与 stable Softmax。主实验仍为 `learning/labs/04_attention_mechanics.ipynb`，互动图 `learning/readings/interactive/core-concepts.html` 可切换 mask；starter 是 `learning/labs/starter/02_causal_attention.py`，完成后运行 `uv run llm-course exercises check 02`。现代 RoPE/GQA starter 在后续周，不要在本周把共享 K/V 混入基准 MHA。
...
在 `learning/labs/04_attention_mechanics.ipynb` 完成 full-vs-causal heatmap、未来 token 扰动和多头 reshape；在 `learning/readings/interactive/core-concepts.html` 逐格切换 mask，确认每行归一化只覆盖允许 key。然后补完 `learning/labs/starter/02_causal_attention.py`，运行 `uv run llm-course exercises check 02`。公共核查通过后，再给自己的实现添加 `Tq=1,Tk=5` 与全遮蔽错误策略测试。
