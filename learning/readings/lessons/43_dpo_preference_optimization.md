...
运行 `learning/labs/10_posttraining.ipynb` 的 sequence log-prob 与 DPO 单元；打开 `learning/readings/interactive/training-and-alignment.html` 切到 DPO。补完 `learning/labs/starter/19_posttraining.py` 的 `sequence_logprob`，执行 `uv run llm-course exercises check 19`。实验扫描 beta 与人为 margin，比较 sum/mean 对长短回答的排序，并构造模板不一致的失败样例。
