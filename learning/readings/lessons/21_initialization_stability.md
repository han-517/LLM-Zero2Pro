...
在 `learning/labs/06_modern_decoder.ipynb` 记录深度方向激活/梯度；在 `learning/readings/interactive/training-and-alignment.html` 观察数据和内存变量；使用 `src/llm_from_scratch/training.py` 的优化器/日志工具做单步 oracle。本周无独立 starter 时，交付 notebook 中的故障注入单元和诊断报告。
