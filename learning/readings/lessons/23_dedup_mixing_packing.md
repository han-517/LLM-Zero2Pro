...
在 `learning/readings/interactive/training-and-alignment.html` 观察过滤保留率；在 `learning/labs/07_pretraining_systems.ipynb` 可视化近重复簇、来源 token 比和 packing 利用率；完成 starter `14` 的去重与数据 pipeline。互动漏斗不展示误删样本，正式报告必须附抽检。
