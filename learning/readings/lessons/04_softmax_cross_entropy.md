...
运行 `learning/labs/01_shapes_and_autograd.ipynb` 的稳定 Softmax 与线性分类器单元，打开 `learning/readings/interactive/core-concepts.html` 查看概率变化。补完 `learning/labs/starter/01_stable_softmax.py`，使用 `uv run llm-course exercises check 01` 核查。阶段概览是 `learning/readings/stages/01_foundations.md`。
