...
使用 `learning/readings/interactive/core-concepts.html#kv-cache` 和 `learning/readings/interactive/architecture-lab.html` 观察长度、层数、KV heads；完成 `learning/labs/starter/03_kv_cache_budget.py`；在 `learning/labs/06_modern_decoder.ipynb` 逐 token 比较 logits。互动图的连续数组不展示分页碎片，分页留到推理阶段。
