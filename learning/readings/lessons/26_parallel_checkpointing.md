...
在 `learning/readings/interactive/training-and-alignment.html` 切换 DDP/ZeRO/FSDP 并观察主导内存；在 `learning/labs/07_pretraining_systems.ipynb` 绘制拓扑、估算通信并做单进程 checkpoint round-trip。本周无多机 starter，交付并行方案和可恢复 manifest。
