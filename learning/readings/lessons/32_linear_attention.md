...
在 `learning/readings/interactive/architecture-lab.html` 选择 Linear state；使用 `learning/labs/08_attention_frontiers.ipynb` 对照 dense、parallel、recurrent 和状态内存；完成 starter `16` 的线性注意力部分。互动图展示固定状态，不展示冲突，实验必须加入 associative recall。
