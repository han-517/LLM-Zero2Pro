...
只需 Python 的 `str`、`bytes`、字典和基础张量。主实验是 `learning/labs/03_tokenization_and_bpe.ipynb`；本周是研究周，没有必填 starter，先不要提前实现第 10 周的 merge。互动入口 `learning/readings/interactive/index.html` 用来查看 token 粒度变化；最可靠的核查仍是标准库 `encode/decode`、十六进制打印和往返断言。
...
打开 `learning/labs/03_tokenization_and_bpe.ipynb`，先只完成 Unicode、UTF-8、字符 tokenizer 与 byte baseline 小节。使用 `learning/readings/interactive/index.html` 的分词视图并排输入英文、中文、组合重音与 ZWJ emoji；记录字符、code point、byte、token 四个计数。本周没有必填 starter；`learning/labs/starter/06_byte_bpe.py` 留到第 10 周。若 notebook 输出与这里不同，优先比较 normalization 与 tokenizer 版本，而不是直接改期望值。
