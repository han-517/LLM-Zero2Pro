...
- [第 1 周：命令行、Git、Python 与可复现环境](../lessons/01_environment_reproducibility.md)
- [第 2 周：向量、矩阵与张量形状](../lessons/02_tensor_shapes.md)
- [第 3 周：导数、链式法则与自动微分](../lessons/03_chain_rule_autograd.md)
- [第 4 周：概率、Softmax、交叉熵与 PyTorch](../lessons/04_softmax_cross_entropy.md)
...
需要会写 Python 函数、循环和类，并能运行仓库测试。不要求熟练微积分证明。环境还未通过时先看[环境搭建](../../../setup/environment.md)，不要在 Notebook 内临时安装另一套 Python。
...
1. 运行 [`core/01_shapes_and_autograd.ipynb`](../../labs/01_shapes_and_autograd.ipynb)，完成形状、广播、分支求和、有限差分和两层网络过拟合。
2. 打开 [Softmax 交互图](../interactive/core-concepts.html#softmax)，先预测共同平移和温度的效果，再拖动滑块。
3. 独立完成 [`01_stable_softmax.py`](../../labs/starter/01_stable_softmax.py)，用大 logits、平移不变性和概率和核查。
