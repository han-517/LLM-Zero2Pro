...
- `starter/`：一个文件中的局部实现题，用公开测试快速定位形状、数值或边界错误。
- [`projects/`](projects/README.md)：多文件集成任务，需要自己连接数据、模型、训练、恢复、
  评测和实验报告；不得导入 `src/` 的参考实现。

先完成当前课列出的 starter，再在课程目录指定的里程碑进入大作业。二者不是重复答案：
局部核查证明组件正确，大作业证明系统能真正运行。
