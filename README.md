...
3. 打开[统一学习路线](docs/learning_path.md)：第一次从第 1 周开始，以后从第一个未完成的周继续。
...
这套课程只有一条：**1 → 48**。所有学习者沿同一顺序推进，可以按自己的时间调整节奏。

```text
# 查看完整路线
uv run llm-course course path
```
...
| 当前情况 | 怎样开始 | 怎样记录 |
|---|---|---|
| 刚接触 LLM | 从第 1 周逐项完成 | 依次更新 1–48 |
| 已掌握一部分 | 先做对应验收，掌握的内容可以快速通过 | 仍依次更新 1–48 |
| 只回来补一个主题 | 定位对应周，复习后回到原进度 | 不创建另一套路径 |

这里的“一周”表示一个学习单元，不强制七天完成。时间少时可以两周完成一周内容；时间充足时也可以一周完成多个学习单元。改变的是进度，不是课程范围。

### 主题索引（仅用于定位）

| 主题 | 对应 Notebook |
|---|---|
| Attention、RoPE、GQA | `04_attention_and_transformer` → `06_modern_architectures` |
| MLA、状态空间模型 | `08_attention_frontiers` |
| MoE | `09_moe` |
| DPO、GRPO | `10_alignment` |
| 推测解码与服务 | `11_inference` |

### 每一周怎样完成

1. 在[统一学习路线](docs/learning_path.md)找到当前周。
2. 打开 `docs/weeks/NN_*.md`，完成阅读和手算。
3. 运行列出的 Notebook；同一 Notebook 可能服务多个周，只完成当前周对应内容。
4. 完成练习或写明的交付物。
5. 运行核查，重启后重新执行一次。
6. 更新进度，再进入下一周。

`course/` 只负责把 48 周连接到材料与验收，不是另一套教程。
...
| 查看 1–48 学习路径 | `uv run llm-course course path` |
